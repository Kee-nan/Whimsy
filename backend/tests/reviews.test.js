...
describe('Reviews and media stats', () => {
  it('adds a review and retrieves it', async () => {
    ...
    const res = await request(app).get('/api/review/get').query({ mediaType: 'movie', id: '7777' }).set('Authorization', `Bearer ${token}`);
    expect(res.status).toBe(200);
    expect(res.body.review.rating).toBe(25);
  });

  it('allows a review with rating 0', async () => {
    const { token } = await createAndLoginUser();
    const reviewData = { id: 'movie/8000', image: '', rating: 0, review: null, title: 'Bad Movie' };
    const res = await request(app).post('/api/review/add').set('Authorization', `Bearer ${token}`).send({ reviewData });
    expect(res.status).toBe(200);
  });
  ...
  it('computes a correct global average across multiple users', async () => {
    ...
    const res = await request(app).get('/api/review/stats').query({ mediaType: 'movie', id: '8888' }).set('Authorization', `Bearer ${tokenA}`);
